...
public class DasTypesAnalyzer  {
    // static String default_das1_registry_sources = "http://www.dasregistry.org/das1/sources";
    // static String default_das1_registry_sources = "file:./data/das1_registry_sources.xm
    static String default_das1_registry_sources = "http://netaffxdas.affymetrix.com/das2/genome"; 

    String das1_sources_query;
    Das1SourcesCapability sources_cap;

    public static void main(String[] args) throws URISyntaxException  {
	DasTypesAnalyzer analyzer = new DasTypesAnalyzer(default_das1_registry_sources);
	analyzer.analyze();
    }

    public DasTypesAnalyzer(String das1_sources_query) throws URISyntaxException  {
	...
	int das2_type_cap_count = 0;
      int das2_type_count = 0;  // count of actual DAS2 types
	...
	for (Das2SourceI source : sources)  {
		...
		Das2TypesCapabilityI das2_types_cap = null;
		...
		for (Das2CapabilityI cap : caps)  {
			...
			type_cap_count++;
                    ...
                    das2_type_cap_count++;
                    Das2TypesCapabilityI das2_type_cap = (Das2TypesCapabilityI)cap;
                    Das2TypesResponseI das2_types_response = das2_type_cap.getTypes();
                    List<Das2TypeI> das2_types = das2_types_response.getTypes();
                    das2_type_count += das2_types.size();
                    System.out.println("DAS2 type count for this version: " + das2_types.size());
		    }
		}
	...
	System.out.println("das1 type cap count: " + type_cap_count);
	System.out.println("das2 type cap count: " + das2_type_cap_count);
        System.out.println("total das2 types: " + das2_type_count);
