...
public class UcscSourcesPlugin implements TrellisSourcesPluginI  {
    static String VERSION = "0.5";  // current version of UcscSourcesPlugin

    UcscSourcesCapability sources_proxy = null;
	...
	boolean success = false;
	    ...
	    sources_proxy = new UcscSourcesCapability(base_uri, sources_query, params);
	    success = true;
        } catch (URISyntaxException ex) {
            Logger.getLogger(UcscSourcesPlugin.class.getName()).log(Level.SEVERE, null, ex);
        }
	return success;
    }
    ...
    public void addHeaders(HttpServletRequest request, HttpServletResponse response) {
        response.addHeader("DAS2-Server", "UcscSourcesPlugin/" + VERSION);
    }
