...
public class SourcesPlugin implements TrellisSourcesPluginI  {
    static String VERSION = "1.0";  // current version of SourcesPlugin

    SourcesCapability sources_cap = null;
	...
	boolean success = false;
	    ...
	    sources_cap = new SourcesCapability(base_uri, sources_query, init_params);
	    success = true;
        } catch (URISyntaxException ex) {
            Logger.getLogger(SourcesPlugin.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(SourcesPlugin.class.getName()).log(Level.SEVERE, null, ex);
        }
	return success;
    ...
    public void addHeaders(HttpServletRequest request, HttpServletResponse response) {
        response.addHeader("DAS2-Server", "SourcesPlugin/" + VERSION);
    }
