...
public class BedFeatureWriter implements Das2FeatureWriterI {
    static Das2Format format = new Das2Format("bed", "text/bed");

    /** for now assuming any type can be represented as BED
	...
	try {
		...
		writeFeature(bw, annot);
            ...
            Logger.getLogger(BedFeatureWriter.class.getName()).log(Level.SEVERE, null, ex);
        }
	return true;
    }
	...
	Das2SegmentI seq = span.getSegment();
        out.write(seq.getLocalURIString());
        out.write('\t');
        int min = span.getMin();
        int max = span.getMax();
